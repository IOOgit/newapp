...
## Коммутатор Dahua DH-CS4226-24ET-240

Эта модель не поддерживает SNMP. Раздел **«Свитчи»** выполняет только проверку
TCP-доступности веб-интерфейса по указанному IP и порту (по умолчанию 80).

Доступный коммутатор показывается жёлтым, а не зелёным: соединение с веб-интерфейсом
не подтверждает состояние физических портов, питание PoE, трафик или ошибки.
Недоступный веб-интерфейс отображается красным. Никакие SNMP community и OID для
этой модели не запрашиваются и не используются.
