...
## Стационарные NVR: запись и готовность хранилища

Общий статус на главной странице и в TV учитывает связь, каналы, HDD, время,
свежесть записи и суточный архив. Зелёный означает успешные актуальные проверки;
неполные, устаревшие и недоступные данные имеют отдельные причины в карточке.
Отключённые каналы не участвуют в оценке. Температура/CPU и качество изображения
учитываются в пределах доступных возможностей и включённых проверок.

Свежий архив проверяется каждые 5 минут: по умолчанию ищется запись в окне
45 минут, допустимый возраст конца записи — 15 минут. Кнопка «Проверить запись»
запускает проверку вручную. Время сравнивается с часами самого регистратора;
сохраняется существующая конвенция местного времени видео без перевода в UTC.
Это проверка индекса архива, а не декодирование видео или гарантия читаемости файла.

В карточке канала задайте режим **непрерывно / по событию / отключено**. По
умолчанию выбран непрерывный режим; для записи по движению обязательно выберите
«по событию», чтобы ожидаемые паузы не считались остановкой. Для сложных расписаний
используйте отключение этой проверки вне непрерывного режима. Ошибка API, часы
NVR недоступны, неподдерживаемый поиск и отсутствие свежей записи различаются.
Настройки периода/окна/допустимого возраста — `RECORDING_CHECK_MINUTES`,
`RECORDING_WINDOW_MINUTES`, `RECORDING_MAX_AGE_MINUTES` в `.env`.

HDD отображает исходный статус и готовность: не инициализирован, только чтение,
неизвестное состояние, отсутствует. При успешном полном опросе ранее известный
диск, исчезнувший из списка, отмечается как пропавший; ошибки опроса не означают
пропажу диска. Процент занятости остаётся справочным, что учитывает циклическую
запись. Новые проверки работают в панели без новых уведомлений Telegram.

## Коммутаторы камер по SNMP

Раздел **«Свитчи»** доступен пользователям с правом мониторинга. Первый профиль
назван **DH-CS4226-24ET-240**. Он использует стандартные MIB; проверка на физическом
свитче этой модели ещё требуется. Поддержка каждого необязательного набора данных
определяется опросом, проприетарные OID не подставляются предположительно.

1. Включите на свитче SNMPv2c для чтения и доступ с IP сервера мониторинга.
2. В панели добавьте имя, адрес, UDP-порт (обычно 161) и community. Секрет шифруется
   существующим ключом приложения и не возвращается в API/HTML; пустое поле при
   редактировании сохраняет прежний секрет.
3. Нажмите «Опросить», затем назначьте камеру соответствующему интерфейсу или
   отметьте порт, где линк должен быть всегда. Свободные down-порты не аварийные.
4. Если доступна стандартная таблица PoE, при необходимости сопоставьте её индекс
   `group.port` с интерфейсом вручную. `ifIndex` не обязательно совпадает с номером
   физического порта; такое соответствие не угадывается автоматически.

Показываются идентификация, время работы SNMP-агента, состояние/скорость
интерфейсов, трафик, прирост ошибок и потерь. По POWER-ETHERNET-MIB дополнительно
доступны состояния PoE и общая мощность PSE. Стандарт не даёт измеренные ватты
каждого порта; отсутствие MIB показывается явно. Неуспешный SNMP-опрос означает
недоступность мониторинга, а не доказанное отключение питания свитча.

Опрос по умолчанию раз в 60 секунд, не более четырёх свитчей параллельно,
с общим таймаутом 30 секунд на устройство. После сброса счётчиков/перезапуска
не строится ложный скачок трафика. Операции SNMP SET и управление питанием портов
не реализованы. Конфигурация, привязки и зашифрованные community включены в бэкап;
для восстановления секретов нужен тот же ключ шифрования приложения.

Стандарты: [System MIB / RFC 3418](https://www.rfc-editor.org/rfc/rfc3418.html),
[IF-MIB / RFC 2863](https://www.rfc-editor.org/rfc/rfc2863.html),
[POWER-ETHERNET-MIB / RFC 3621](https://www.rfc-editor.org/rfc/rfc3621.html).
